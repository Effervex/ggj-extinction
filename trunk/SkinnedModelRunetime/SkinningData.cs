...
namespace AlphaSubmarines
{
    ...
    public class SkinningData
    {
        ...
        public SkinningData(Dictionary<string, AnimationClip> animationClips,
                            Matrix[] inverseBindPose,Matrix[] bindPose,
                            int[] skeletonHierarchy, string[] boneNames)
        {
            int s = 0;
            this.AnimationClips = new AnimationClip[animationClips.Count];
            this.AnimationClipsNames = new string[animationClips.Count];
            foreach (KeyValuePair<string, AnimationClip> k in animationClips)
            {
                this.AnimationClips[s] = k.Value;
                this.AnimationClipsNames[s] = k.Key;
                s++;
            }
                ...
                for (int i = 0; i < AnimationClips.Length; i++)
                {
                    ...
                    for (int j = 0; j < AnimationClips[i].Keyframes.Length; j++)
                    {
                        ...
                        if (k0 != null && k1 != null)
                        {
                            ...
                            if (minDuration < FrameStep)
                            {
                                FrameStep = minDuration;
                            }
                            k0 = null;
                            k1 = null;
                        }
                    ...
                    //This operation is done at model compile time so we don't chare about speed.
        ...
        {
            for (int i = 0; i < AnimationClipsNames.Length; i++)
            {
                if (clipName == AnimationClipsNames[i])
                {
        ...
        public static SkinningData Empty
        {
            get
            {
                ...
                {
                    emptySkiningData = new SkinningData(new Dictionary<string, AnimationClip>(), new Matrix[1] { Matrix.Identity }, new Matrix[1] { Matrix.Identity }, new int[1]{0}, new string[1]{"root"});
                }
